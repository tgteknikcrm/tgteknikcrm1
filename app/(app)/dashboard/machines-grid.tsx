...
export function MachinesGrid({ cards }: { cards: MachineCardData[] }) {
  ...
  const list = editMode ? cards : cards.filter((c) => !hidden.has(c.machine.id));
    ...
    <>
      ...
      {editMode && (
        <p className="mb-3 text-xs text-muted-foreground">
          Düzenleme modundasın. Her kartın sağ üstündeki göz ikonuna tıklayarak
          o kartı gizleyebilir veya geri açabilirsin. Bittiğinde
      ...
      <div className="grid auto-rows-fr grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {list.map((c) => {
          ...
          return (
            <div key={c.machine.id} className="relative h-full">
              {editMode && (
                <button
                  type="button"
